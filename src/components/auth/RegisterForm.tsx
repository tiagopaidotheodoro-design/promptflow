...
export function RegisterForm() {
  ...
  const strongPass = form.password.length >= 6;
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        {form.password.length > 0 && (
          <p className={`text-xs flex items-center gap-1 ${strongPass ? "text-brand-green-neon" : "text-text-muted"}`}>
            <Check className="h-3 w-3" />
            {strongPass ? "Senha forte" : "Muito curta (mínimo 6 caracteres)"}
          </p>
        )}
      </div>
      ...
      <p className="text-center text-xs text-text-muted">
        ...
        <a href="/legal/termos" className="text-brand-purple hover:underline">Termos de Uso</a>
        {" "}e{" "}
        <a href="/legal/privacidade" className="text-brand-purple hover:underline">Política de Privacidade</a>.
      </p>
    </form>
  );
}
